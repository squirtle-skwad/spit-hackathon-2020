...
import { ListGroup, ListGroupItem, Badge, Row, Col, Card, CardText, CardDeck, CardTitle, CardColumns } from 'reactstrap';
      ...
      {(userRole === "volunteer" ?
        (
          <>
            ...
            <Query query={queries.GET_ACTIONS} variables={{ volunteerId: userId }}>{
              ({ loading, error, data }) => {
                if (loading) {
                  return <LoadingPopup />
                }
                    ...
                    <CardColumns>
                      {
                        data.donation_volunteer.map((donation) => {
                          return (
                            <Card tag="a" onClick={() => onCardClick(donation.donation_request_id)} style={{ cursor: "pointer", margin: "2rem", width: "20rem" }} body outline color="primary">
                              <Row>
                                <Col>
                                  <TestGoogleMap lat={donation.donation_request.latitude} lng={donation.donation_request.longitude} placeName={donation.donation_request.donor.name} />
                                </Col>
                                <Col>
                                  <CardText><h4>{(donation.assigned ? <Badge style={{ marginRight: "1rem" }}>Assigned</Badge> : <Badge style={{ marginRight: "1rem", color: "success" }}>Unassigned</Badge>)}</h4></CardText>
                                </Col>
                              </Row>
                            </Card>
                          )
                          ...
                          return (
                            <Card tag="a" onClick={() => onCardClick(donation.donation_request_id)} style={{ cursor: "pointer", margin: "2rem", width: "20rem" }} body outline color="primary">
                              <Row>
                                ...
                                <Col>
                                  <CardText><h4><Badge style={{ marginRight: "1rem" }} >Role</Badge>{donation.role}<br /><br />{(donation.donation_request.is_assigned ? <Badge style={{ marginRight: "1rem" }}>Assigned</Badge> : <Badge style={{ marginRight: "1rem", color: "success" }}>Unassigned</Badge>)}</h4></CardText>
                                </Col>
                              </Row>
                            </Card>
                          )
